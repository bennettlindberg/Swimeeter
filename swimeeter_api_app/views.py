...
class Event_view(APIView):
    def get(self, request):
        ...
        if 'specific_to' in request.data and request.data['specific_to'] == 'meet':
            ...
            if 'meet_id' not in request.data:
                return Response({'get_success': False, 'reason': 'no meet id passed'}, status = status.HTTP_400_BAD_REQUEST)

            events_of_meet = Event.objects.filter(meet_id = request.data['meet_id'])
            events_of_meet_JSON = json.loads(serialize("json", events_of_meet, fields = ['id', 'stroke', 'distance', 'competing_gender', 'competing_max_age', 'competing_min_age']))
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({'post_success': False, 'reason': 'invalid creation data passed'}, status = status.HTTP_400_BAD_REQUEST)

        new_event_JSON = json.loads(serialize("json", [new_event], fields = ['id', 'stroke', 'distance', 'competing_gender', 'competing_max_age', 'competing_min_age']))[0]
...
class Swimmer_view(APIView):
    def get(self, request):
        ...
        if 'specific_to' in request.data and request.data['specific_to'] == 'meet':
            ...
            swimmers_of_meet_JSON = json.loads(serialize("json", swimmers_of_meet, fields = ['id', 'first_name', 'last_name', 'age', 'gender', 'team']))
    ...
    def post(self, request):
        ...
        new_swimmer_JSON = json.loads(serialize("json", [new_swimmer], fields = ['id', 'first_name', 'last_name', 'age', 'gender', 'team']))[0]
...
class Entry_view(APIView):
    def get(self, request):
        ...
        if 'specific_to' in request.data and request.data['specific_to'] == 'meet_event':
            # ? no meet id passed
            if 'meet_id' not in request.data:
                return Response({'get_success': False, 'reason': 'no meet id passed'}, status = status.HTTP_400_BAD_REQUEST)
            # ? no event id passed
            if 'event_id' not in request.data:
                return Response({'get_success': False, 'reason': 'no event id passed'}, status = status.HTTP_400_BAD_REQUEST)

            entries_of_meet_event = Entry.objects.filter(event_id = request.data['event_id'], event__meet_id = request.data['meet_id'])
            entries_of_meet_event_JSON = json.loads(serialize("json", entries_of_meet_event, fields = ['id', 'seed_time', 'swimmer']))
            return Response({'get_success': True, 'data': entries_of_meet_event_JSON})
        
        # get all swimmers for a specific meet swimmer
        elif 'specific_to' in request.data and request.data['specific_to'] == 'meet_swimmer':
            # ? no meet id passed
            if 'meet_id' not in request.data:
                return Response({'get_success': False, 'reason': 'no meet id passed'}, status = status.HTTP_400_BAD_REQUEST)
            # ? no swimmer id passed
            if 'swimmer_id' not in request.data:
                return Response({'get_success': False, 'reason': 'no swimmer id passed'}, status = status.HTTP_400_BAD_REQUEST)

            entries_of_meet_swimmer = Entry.objects.filter(swimmer_id = request.data['swimmer_id'], swimmer__meet_id = request.data['meet_id'])
            entries_of_meet_swimmer_JSON = json.loads(serialize("json", entries_of_meet_swimmer, fields = ['id', 'seed_time', 'event']))
            return Response({'get_success': True, 'data': entries_of_meet_swimmer_JSON})
    ...
    def post(self, request):
        ...
        new_entry_JSON = json.loads(serialize("json", [new_entry], fields = ['id', 'seed_time', 'swimmer', 'event']))[0]
...
class Meet_view(APIView):
    def get(self, request):
        ...
        if 'specific_to' in request.data and request.data['specific_to'] == 'all':
            ...
            meets_of_all_JSON = json.loads(serialize("json", meets_of_all, fields = ['id', 'name', 'lanes', 'measure_unit', 'host']))
        ...
        # get all meets for a specific host
            ...
            if 'host_id' not in request.data:
                return Response({'get_success': False, 'reason': 'no host id passed'}, status = status.HTTP_400_BAD_REQUEST)

            meets_of_host = Meet.objects.filter(host_id = request.data['host_id'])
            meets_of_host_JSON = json.loads(serialize("json", meets_of_host, fields = ['id', 'name', 'lanes', 'measure_unit']))
    ...
    def post(self, request):
        ...
        new_meet_JSON = json.loads(serialize("json", [new_meet], fields = ['id', 'name', 'lanes', 'measure_unit']))[0]
...
class Heat_view(APIView):
    def get(self, request):
        ...
        if 'specific_to' in request.data and request.data['specific_to'] == 'meet_event':
            ...
            heats_of_meet_event = Heat.objects.filter(event_id = request.data['event_id'], event__meet_id = request.data['meet_id'])
            heats_of_meet_event_JSON = json.loads(serialize("json", heats_of_meet_event, fields = ['id', 'order_in_event']))
...
class Heat_assignment_view(APIView):
    def get(self, request):
        ...
        if 'specific_to' in request.data and request.data['specific_to'] == 'meet_event_heat':
            # ? no meet id passed
            if 'meet_id' not in request.data:
                return Response({'get_success': False, 'reason': 'no meet id passed'}, status = status.HTTP_400_BAD_REQUEST)
            # ? no event id passed
            ...
            if 'heat_id' not in request.data:
                return Response({'get_success': False, 'reason': 'no heat id passed'}, status = status.HTTP_400_BAD_REQUEST)

            assignments_of_meet_event_heat = HeatLaneAssignment.objects.filter(heat_id = request.data['heat_id'], heat__event_id = request.data['event_id'], heat__event__meet_id = request.data['meet_id'])
            assignments_of_meet_event_heat_JSON = json.loads(serialize("json", assignments_of_meet_event_heat, fields = ['id', 'lane', 'entry']))
            return Response({'get_success': True, 'data': assignments_of_meet_event_heat_JSON})
